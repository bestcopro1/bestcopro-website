...
{
    $request =
        "SELECT id FROM proprietaire WHERE prenom LIKE ? AND nom LIKE ? AND (telephone LIKE ? OR mobile LIKE ?) LIMIT 1";
    if ($stmt = $connection->prepare($request)) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($_FILES["file"]["error"] !== UPLOAD_ERR_OK) {
        ...
        echo $uploadErrors[$_FILES["file"]["error"]] ??
            "Erreur inconnue pendant l'import du fichier.";
        exit();
    }
    if ($_FILES["file"]["name"] == "") {
    ...
    if ($_FILES["file"]["name"] != "" && $_POST["id_copropriete"] != "") {
        ...
        if (($handle = fopen($location, "r")) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                ...
                if (
                    $data[0] != "Appartement" &&
                    $data[0] != "Local commercial" &&
                    $data[0] != "Magasin" &&
                    $data[0] != "Villa"
                ) {
                    echo "Veuillez vous assurer du type du lot dans la ligne : " .
                        $counter;
                    exit();
                } elseif ($data[1] == "") {
                    echo "Veuillez vous assurer du code du lot dans la ligne : " .
                        $counter;
                    exit();
                } elseif ($data[2] == "") {
                    echo "Veuillez vous assurer de l'immeuble dans la ligne : " .
                        $counter;
                    exit();
                } elseif ($data[3] == "" || !is_numeric($data[3])) {
                    echo "Veuillez vous assurer de l'étage dans la ligne : " .
                        $counter .
                        "<br>L'étage doit être un entier supérieur ou égal à 0";
                    exit();
                } elseif ($data[4] == "") {
                    echo "Veuillez vous assurer du numéro du lot dans la ligne : " .
                        $counter;
                    exit();
                } elseif (
                    trim($data[6]) != "" &&
                    (count(explode("/", trim($data[6]))) != 3 ||
                        strlen(explode("/", trim($data[6]))[0]) != 2 ||
                        strlen(explode("/", trim($data[6]))[1]) != 2 ||
                        strlen(explode("/", trim($data[6]))[2]) != 4 ||
                        !checkdate(
                            explode("/", trim($data[6]))[1],
                            explode("/", trim($data[6]))[0],
                            explode("/", trim($data[6]))[2],
                        ))
                ) {
                    echo "Veuillez vous assurer de la date d'acquisition dans la ligne : " .
                        $counter;
                    exit();
                } elseif (
                    $data[7] == "" ||
                    ...
                    echo "Veuillez vous assurer du tantième dans la ligne : " .
                        $counter .
                        "<br>Le tantième doit être un nombre flottant non nul";
                    exit();
                } elseif ($data[14] != "Résident" && $data[14] != "Promoteur") {
                    echo "Veuillez vous assurer du type du propriétaire dans la ligne : " .
                        $counter;
                    exit();
                } elseif ($data[16] == "") {
                    echo "Veuillez vous assurer du nom du propriétaire dans la ligne : " .
                        $counter;
                    exit();
                } elseif ($data[17] == "") {
                    echo "Veuillez vous assurer du prénom du propriétaire dans la ligne : " .
                        $counter;
                    exit();
                }
            }
        ...
        if (($handle = fopen($location, "r")) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                ...
                $codeHtml .=
                    ...
                    "</span>";
                $codeHtml .=
                    '<input type="hidden" name="id_lot_' .
                    $counter .
                    '" value="' .
                    $id_lot .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="id_typeLot_' .
                    $counter .
                    '" value="' .
                    $id_typeLot .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="numeroImm_' .
                    $counter .
                    '" value="' .
                    trim($data[2]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="etage_' .
                    $counter .
                    '" value="' .
                    trim($data[3]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="numero_' .
                    $counter .
                    '" value="' .
                    trim($data[4]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="foncier_' .
                    $counter .
                    '" value="' .
                    trim($data[5]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="tantieme_' .
                    $counter .
                    '" value="' .
                    trim($data[7]) .
                    '" class="tantieme">';
                $codeHtml .=
                    '<input type="hidden" name="dateAcquisition_' .
                    $counter .
                    '" value="' .
                    $dateAcquisition .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="dateRemiseCle_' .
                    $counter .
                    '" value="' .
                    $dateRemiseCle .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="id_proprietaire_' .
                    $counter .
                    '" value="' .
                    $id_proprietaire .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="id_typeProprietaire_' .
                    $counter .
                    '" value="' .
                    $id_typeProprietaire .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye0_' .
                    $counter .
                    '" value="' .
                    trim($data[13]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye1_' .
                    $counter .
                    '" value="' .
                    trim($data[12]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye2_' .
                    $counter .
                    '" value="' .
                    trim($data[11]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye3_' .
                    $counter .
                    '" value="' .
                    trim($data[10]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye4_' .
                    $counter .
                    '" value="' .
                    trim($data[9]) .
                    '">';
                $codeHtml .=
                    '<input type="hidden" name="impaye5_' .
                    $counter .
                    '" value="' .
                    trim($data[8]) .
                    '">';
                ...
                $codeHtml .=
                    ...
                    "</span></td>";
                $codeHtml .=
                    '<td><span id="tdRemise_' .
                    $counter .
                    '">' .
                    trim($data[6]) .
                    "</span></td>";
                $codeHtml .=
                    '<td><a href="#" class="btn btn-primary shadow btn-xs sharp me-1 edit_lot" data-lot-line="' .
                    $counter .
                ...
                $codeHtml2 .=
                    ...
                    "</span>";
                $codeHtml2 .=
                    '<input type="hidden" name="id_lot2_' .
                    $counter .
                    '" value="' .
                    $id_lot .
                    '">' .
                ...
                $codeHtml2 .=
                    ...
                    "</span></td>";
                $codeHtml2 .=
                    '<td><span id="tdTantieme2_' .
                    $counter .
                    '">' .
                    trim($data[7]) .
                    "</span></td>";
                $codeHtml2 .=
                    '<td><input type="number" class="form-control input-default partFonct" name="partFonct_' .
                    $counter .
                    '" value="' .
                    trim($data[7]) .
                    '" placeholder="0.00" readonly></td>';
                $codeHtml2 .=
                    '<td><input type="number" class="form-control input-default partInv" name="partInv_' .
                    $counter .
                    '" value="' .
                    trim($data[7]) .
                    '" placeholder="0.00" readonly></td>';
