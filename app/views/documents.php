...
if (isset(
		$_POST['typedocument'],
		$_POST['update_typedocument']
	)) {
	$error_msg = "";
		
	$typedocument = filter_input(INPUT_POST, 'typedocument', FILTER_SANITIZE_STRING);	
	$update_typedocument = filter_input(INPUT_POST, 'update_typedocument', FILTER_SANITIZE_STRING);	
	...
	if ($update_typedocument == "true") {
		...
		if (empty($error_msg)) {
			...
			echo "done|".$insert_id."|<option value='".$insert_id."'>".$typedocument."</option>";
			exit();
		} else {
			echo $error_msg;
			exit();
    ...
    $id_typedocument = filter_input(INPUT_POST, 'id_typedocument', FILTER_SANITIZE_STRING);
	...
	if (empty($error_msg) && isset($_POST['id'],$_POST['update'])) {
		...
		if ($id != '' && $update == 'true') {
			...
			$request = "UPDATE document SET titre=?, date=?, id_typedocument=?, public=? WHERE id=?";

			if ($insert_stmt = $connection->prepare($request)) {
				$insert_stmt->bind_param('sssss', $titre, $date, $id_typedocument, $public, $id);
				// Execute the prepared query.
				if (! $insert_stmt->execute()) {
					echo $connection->error;
					exit();
			...
			if(isset($_FILES['file']['name'])){
				if ($_FILES['file']['name'] != "") {
					...
					$location = __DIR__."/../justificatifs/documents/".$id.".".$fileType;
					$oldFiles = glob("../justificatifs/documents/".$id.".*");
					...
					$linkFile = getURL()."/../justificatifs/documents/".$id.".".$fileType;
				}
			}
		...
		if ($insert_stmt = $connection->prepare($request)) {
            $insert_stmt->bind_param('sssss', $titre, $date, $id_typedocument, $id_copropriete, $public);
            // Execute the prepared query.
            if (! $insert_stmt->execute()) {
				echo $connection->error;
                exit();
		...
		if(isset($_FILES['file']['name'])){
			if ($_FILES['file']['name'] != "") {
				$fileType = pathinfo($_FILES['file']['name'],PATHINFO_EXTENSION);
				$fileType = strtolower($fileType);
				$location = __DIR__."/../justificatifs/documents/".$insert_id.".".$fileType;
				move_uploaded_file($_FILES['file']['tmp_name'],$location);
				$linkFile = getURL()."/../justificatifs/documents/".$insert_id.".".$fileType;
			}
		}
...
if (isset($_GET['action'],$_GET['id'])):
	...
	if (count($document) == 0) goto iomEnd;
...
?>
		<div class="content-body">
			...
			<div class="container-fluid">
										...
										<div class="row">
											<div class="col-6 mb-2" >
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="titre" placeholder="Titre du document" value="<?=$document[0]["titre"]?>" <?=$disabled?>>
												</div>
											</div>
											<div class="col-6 mb-2" >
                                                <div class="form-group">
													<label class="text-label">Date*</label>
													<input type="date" class="form-control input-rounded input-default mb-3" name="date" placeholder="Date" value="<?=$document[0]["date"]?>" <?=$disabled?>>
												</div>
											</div>
											<div class="col-6 mb-2" >
												<div class="form-group">
														...
														<div class="col-10">
															<span id="id_typedocumentContainer">
																<select name="id_typedocument" id="id_typedocument" class="default-select form-control input-rounded wide mb-3" <?=$disabled?>>
																	...
																	?>
																	<option value="<?=$typedocument["id"]?>" <?php if($typedocument["id"] == $document[0]["id_typedocument"]) echo "selected";?>><?=$typedocument["libelle"]?></option>
																	<?php
																	endforeach;
																	?>
																</select>
											...
											<div class="col-6 mb-2" >
                                                <div class="form-group">
													...
													<div class="mb-0">
														<label class="radio-inline me-3"><input type="radio" name="public" value="1" class="form-check-input" <?php if ($document[0]["public"] == "1") echo "checked";?> <?=$disabled?>> Oui</label>
														<label class="radio-inline me-3"><input type="radio" name="public" value="0" class="form-check-input" <?php if ($document[0]["public"] == "0") echo "checked";?> <?=$disabled?>> Non</label>
													</div>
												</div>
											</div>
                                        </div>
				...
				<?php
				$preuves = glob ("./justificatifs/documents/".$document[0]["id"].".*");
				?>
...
<?php
	...
	iomEnd:
	$documents = getDocument(null, $GLOBALS["id_copropriete"], null, $connection);
?>
		<div class="content-body">
            <!-- row -->
			<div class="container-fluid">
                                    ...
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <tbody>
											...
											<tr>
                                                ...
                                                <td><?=$typedocument[0]["libelle"]?></td>
                                                <td class="text-center">
													<?php
													if ($_SESSION['id_usertype'] === "1" || $_SESSION['id_usertype'] === "2" || $_SESSION['id_usertype'] === "3") :
													?>
													<a href="./dashboard.php?page=documents&action=update&id=<?=$document["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
													<?php
													else :
													?>
													<a href="./dashboard.php?page=documents&action=update&id=<?=$document["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>
													<?php
													endif;
													?>
												</td>
