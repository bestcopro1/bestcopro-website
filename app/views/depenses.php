...
if (isset($_POST['select'])) {
	...
	if ($id_rubrique != "" || $id_rubrique != null) {
		...
		foreach($postes as $poste) {
			$codeHtml .= '<option value="'.$poste["id"].'">'.$poste["libelle"].'</option>';
		}
		echo 'done|'.$codeHtml;
		exit();
	}
    ...
    $id_fournisseur = filter_input(INPUT_POST, 'id_fournisseur', FILTER_SANITIZE_STRING);
	...
	$request = "SELECT id_typeRubrique FROM rubrique WHERE id = (SELECT id_rubrique FROM poste WHERE id = ?)";
	...
	$request = "select COALESCE(SUM(montant),0) from depense where id_exercice = ? AND id_poste IN (SELECT id FROM poste WHERE id_rubrique IN (SELECT id FROM rubrique WHERE id_typeRubrique = ?))";
	if ($stmt = $connection->prepare($request)) {
	...
	if ($id_typeRubrique == "1" && (floatval($montant) + floatval($totalDepenses)) > floatval($exercice[0]["montantFonct"])) {
		$error_msg .= 'Vous avez dépassé le budget de fonctionnement';
		echo $error_msg;
		exit();
	}
	if ($id_typeRubrique == "2" && (floatval($montant) + floatval($totalDepenses)) > floatval($exercice[0]["montantInvest"])) {
	...
	if (empty($error_msg) && isset($_POST['id'],$_POST['update'])) {
		...
		if ($id != '' && $update == 'true') {
			...
			$request = "UPDATE depense SET id_poste=?, date=?, montant=?, id_fournisseur=?, id_modePaiement=?, commentaire=? WHERE id=?";

			if ($insert_stmt = $connection->prepare($request)) {
				$insert_stmt->bind_param('sssssss', $id_poste, $date, $montant, $id_fournisseur, $id_modePaiement, $commentaire, $id);
				// Execute the prepared query.
				if (! $insert_stmt->execute()) {
					echo $connection->error;
					exit();
			...
			if(isset($_FILES['file']['name'])){
				if ($_FILES['file']['name'] != "") {
					...
					$location = __DIR__."/../justificatifs/depenses/".$id.".".$fileType;
					...
					$linkFile = getURL()."/../justificatifs/depenses/".$id.".".$fileType;
				}
			}
		...
		if ($insert_stmt = $connection->prepare($request)) {
            $insert_stmt->bind_param('ssssssss', $id_poste, $date, $montant, $id_fournisseur, $id_modePaiement, $commentaire, $id_exercice, $id_syndic);
            // Execute the prepared query.
            if (! $insert_stmt->execute()) {
				echo $connection->error;
                exit();
		...
		if(isset($_FILES['file']['name'])){
			if ($_FILES['file']['name'] != "") {
				$fileType = pathinfo($_FILES['file']['name'],PATHINFO_EXTENSION);
				$fileType = strtolower($fileType);
				$location = __DIR__."/../justificatifs/depenses/".$insert_id.".".$fileType;
				move_uploaded_file($_FILES['file']['tmp_name'],$location);
				$linkFile = getURL()."/../justificatifs/depenses/".$insert_id.".".$fileType;
			}
		}
...
if (isset($_GET['action'],$_GET['id'])):
	...
	if (count($depense) == 0 || ($_SESSION['id_usertype'] !== "1" && $_SESSION['id_usertype'] !== "2" && $_SESSION['id_usertype'] !== "3")) goto iomEnd;
...
?>
		<div class="content-body">
			...
			<div class="container-fluid">
                                    ...
                                    <form>
                                            ...
                                            <div class="col-6 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_poste" id="id_poste" class="single-select2 form-control wide mb-3">
                                                        <?php
                                                        $poste = getPoste($depense[0]["id_poste"], null, null, $connection);
                                                        ?>
                                                        <option value="<?=$poste[0]["id"]?>"><?=$poste[0]["libelle"]?></option>
                                                    </select>
                                                </div>
                                            </div>
											<div class="col-6 mb-2">
												<div class="form-group">
                                                    ...
                                                    <input type="date" class="form-control input-rounded input-default mb-3" name="date" placeholder="jj/mm/aaaa" value="<?=$depense[0]["date"]?>">
                                                </div>
                                            </div>
											<div class="col-6 mb-2">
												<div class="form-group">
													<label class="text-label">Montant*</label>
													<input type="text" class="form-control input-rounded input-default mb-3" name="montant" placeholder="Montant" value="<?=$depense[0]["montant"]?>">
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<select name="id_fournisseur" class="single-select2 form-control wide mb-3">
                                                        ...
                                                        ?>
                                                        <option value="<?=$fournisseur["id"]?>" <?php if($fournisseur["id"] == $depense[0]["id_fournisseur"]) echo "selected"; ?>><?=$fournisseur["raisonSocial"]?></option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
                                            ...
                                            <div class="col-6 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_modePaiement" class="default-select form-control input-rounded wide mb-3">
                                                        ...
                                                        ?>
                                                        <option value="<?=$modepaiement["id"]?>" <?php if($modepaiement["id"] == $depense[0]["id_modePaiement"]) echo "selected"; ?>><?=$modepaiement["libelle"]?></option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
											...
											<div class="col-6 mb-2">
												<div class="form-group">
                                                    ...
                                                    <textarea class="form-control input-rounded input-default mb-3" name="commentaire" placeholder="Commentaire"><?=$depense[0]["commentaire"]?></textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
				...
				<?php
				$preuves = glob ("./justificatifs/depenses/".$depense[0]["id"].".*");
				?>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_poste" id="id_poste" class="single-select2 form-control wide mb-3">
                                                        <?php
                                                        $postes = getPoste(null, $rubriques[0]["id"], null, $connection);
                                                        foreach($postes as $poste):
                                                        ?>
											...
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<select name="id_fournisseur" class="single-select2 form-control wide mb-3">
                                                        ...
                                                        foreach($fournisseurs as $fournisseur):
                                                        ?>
                                                        <option value="<?=$fournisseur["id"]?>"><?=$fournisseur["raisonSocial"]?></option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
		...
		<div class="content-body">
			...
			<div class="container-fluid">
				<div class="form-head d-flex mb-3 align-items-start">
					...
					?>
					<a href="export/export_depense.php?id_copropriete=<?=$GLOBALS["id_copropriete"]?>&id_exercice=<?=$GLOBALS["id_exercice"]?>" type="button" class="btn btn-rounded btn-primary me-2">
                                        ...
                                        <?php
                                            ...
                                            <tr class="trDepense-<?=$depense["id"]?>">
												...
												<td class="text-center">
													<a href="./dashboard.php?page=depenses&action=update&id=<?=$depense["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
													<a href="javascript:void(0);" class="btn btn-secondary shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target=".delDepense-<?=$depense["id"]?>"><i class="fas fa-trash"></i></a>
												</td>
												<?php
												endif;
												?>
