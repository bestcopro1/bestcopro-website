...
if (
    ...
    if (empty($error_msg) && isset($_POST["id"], $_POST["update"])) {
        ...
        if ($id != "" && $update == "true") {
            $request =
                "UPDATE echeance SET description=?, date=?, jourAlerte=? WHERE id=?";
            ...
            $request =
                "DELETE FROM notificationsyndic WHERE idPage = ? AND nomPage = 'echeances'";
            if ($insert_stmt = $connection->prepare($request)) {
...
if (isset($_GET["action"], $_GET["id"])):
    ...
    if ($action == "update" && $id != ""):
        ...
        $request =
            "UPDATE notificationsyndic SET seen = 1 WHERE idPage = ? AND nomPage = 'echeances'";
        if ($insert_stmt = $connection->prepare($request)) {
            $insert_stmt->bind_param("s", $id);
            // Execute the prepared query.
            if (!$insert_stmt->execute()) {
         ...
         $_SESSION["id_usertype"] === "3"
                ...
                <div class="row">
                 ...
                 "id_copropriete"
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="description" placeholder="Description" value="<?= $echeance[0][
                 "description"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2" >
                                                <div class="form-group">
													...
													<input type="date" class="form-control input-rounded input-default mb-3" name="date" placeholder="jj/mm/aaaa" value="<?= $echeance[0][
                 "date"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
                                            <div class="col-6 mb-2" >
                                                <div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="jourAlerte" placeholder=" M'avertir X jours avant la date d'écheance" value="<?= $echeance[0][
                 "jourAlerte"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
                                    ...
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <?php foreach (
                                            ...
                                            <tr>
												...
												<td><?= $echeance["jourAlerte"] ?> jour(s) avant le <?= date(
     "d/m/Y",
     strtotime($echeance["date"]),
 ) ?></td>
                 ...
                 $_SESSION["id_usertype"] === "3"
													...
													<a href="./dashboard.php?page=echeances&action=update&id=<?= $echeance[
                 "id"
