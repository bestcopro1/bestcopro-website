...
function getImpaye($id_lot, $id_paiement = null, $connection){
	if ($id_paiement != null)
		$request = "SELECT id_rel, id_lot, id_exercice, partFonct, partInv, dateFinPeriode, cotisation FROM rel_lot_exercice WHERE id_lot = ? AND id_rel IN (SELECT id_rel FROM rel_rel_paiement WHERE id_paiement = ?) ORDER BY dateFinPeriode ASC";
	elseif ($id_lot != null)
		$request = "SELECT id_rel, id_lot, id_exercice, partFonct, partInv, dateFinPeriode, cotisation FROM rel_lot_exercice WHERE (partFonct + partInv) > cotisation AND id_lot = ? ORDER BY dateFinPeriode ASC";
...
if (isset(
	...
	if ($id != '' && $printZone == 'true') {
		...
		if (count($paiement) > 0) {
			...
			$codeHtml .= '<strong>Reçu de paiement N° : '.date("y").sprintf("%'.05d", $paiement[0]["id"]).'</strong>';
			$codeHtml .= '<span class="float-end">'.$copropriete[0]["ville"].' le '.date("d/m/Y").'</span>';
			$codeHtml .= '</div>';
			$codeHtml .= '<div class="card-body">';
			$codeHtml .= '<div class="row mb-0">';
			$codeHtml .= '<div class="mt-2 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6">';
			$codeHtml .= '<div class="row align-items-center">';
			$codeHtml .= '<div class="col-sm-12">';
			$codeHtml .= '<div class="brand-logo mb-3">';
			$codeHtml .= '<img class="logo-abbr me-2" width="50" src="images\logo.png" alt="">';
			$codeHtml .= '<img class="brand-title d-inline" width="110" src="images\logo-text.png" alt="">';
			$codeHtml .= '</div>';
			$codeHtml .= '<!--<span>+212 66 03 010 51 / +212 66 36 376 20</span><br>';
			...
			$codeHtml .= '</div>';
			$codeHtml .= '<div class="mt-2 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6">';
			$codeHtml .= '<div><strong>'.$proprietaire[0]["civilite"].' '.$proprietaire[0]["prenom"].' '.$proprietaire[0]["nom"].'</strong></div>';
			$codeHtml .= '<div>Copropriété : '.$copropriete[0]["nom"].'</div>';
			$codeHtml .= '<div>'.getTypelot($lot[0]["id_typeLot"], $connection)[0]["libelle"].' N° : '.$lot[0]["numero"].'</div>';
			$codeHtml .= '<div>Immeuble N° : '.$lot[0]["numeroImm"].'</div>';
			$codeHtml .= '</div>';
			$codeHtml .= '</div>';
			$codeHtml .= '<hr>';
			...
			$codeHtml .= '<p class="font-w400 mb-2 d-flex"><span class="custom-label-2">Montant total : </span><span class="font-w600">'.number_format(floatval($paiement[0]["montant"]), 2, ",", " ").' MAD</span></p>';
			$codeHtml .= '<p class="font-w400 mb-2 d-flex"><span class="custom-label-2">Mode de paiement : </span><span class="font-w600">'.$modepaiement[0]["libelle"].'</span></p>';
			$codeHtml .= '<p class="font-w400 mb-2 d-flex"><span class="custom-label-2">Périodes associées : </span>';
			$codeHtml .= '<div class="table-responsive" style="margin-top: -20px;">';
			$codeHtml .= '<table class="table table-bordered table-responsive-sm">';
			$codeHtml .= '<tbody>';
			$codeHtml .= '<tr>';
			...
			if(count($relRelPaiements) <= 6) {
				...
				foreach ($relRelPaiements as $relRelPaiement) {
					...
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;"><strong>'.$periodeInfo[0]["nomPeriode"].'</strong></td></p>';
					$TotalRelPaiement += floatval($relRelPaiement["montant"]);
				}
				if (floatval($paiement[0]["montant"]) > $TotalRelPaiement) 
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;"><strong>Avance</strong></td>';
				$codeHtml .= '</tr>';
				$codeHtml .= '<tr>';
				foreach ($relRelPaiements as $relRelPaiement) {
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;">'.number_format(floatval($relRelPaiement["montant"]), 2, ",", " ").'</td>';
				}
				if (floatval($paiement[0]["montant"]) > $TotalRelPaiement) 
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;">'.number_format(floatval($paiement[0]["montant"]) - $TotalRelPaiement, 2, ",", " ").'</td>';
			} else {
				$TotalRelPaiement = 0;
				foreach ($relRelPaiements as $relRelPaiement) {
					$TotalRelPaiement += floatval($relRelPaiement["montant"]);
				...
				for ($i = 0; $i < 6; $i++) {
					$periodeInfo = periodeInfo($relRelPaiements[$i]["id_rel"], $connection);
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;"><strong>'.$periodeInfo[0]["nomPeriode"].'</strong></td></p>';
				}
				$codeHtml .= '</tr>';
				$codeHtml .= '<tr>';
				for ($i = 0; $i < 6; $i++) {
					$codeHtml .= '<td class="text-center font-w400 p-2" style="font-size: 10px;">'.number_format(floatval($relRelPaiements[$i]["montant"]), 2, ",", " ").'</td>';
				}
				$codeHtml .= '</tr>';
				$codeHtml .= '</tbody>';
				$codeHtml .= '</table>';
			...
			$codeHtml .= '<div class="col-lg-4 col-sm-5 col-6 ms-auto pt-2 pb-5">';
			$codeHtml .= 'Signature';
			$codeHtml .= '</div>';
			$codeHtml .= '</div>';
			$codeHtml .= '</div>';
	...
	if ($id != '' && $delete == 'true') {
		...
		foreach($relRelPaiements as $relRelPaiement){	
			$request = "UPDATE rel_lot_exercice SET cotisation = (cotisation - ? ) WHERE id_rel = ?";
			if ($insert_stmt = $connection->prepare($request)) {
				$insert_stmt->bind_param('ss', $relRelPaiement["montant"], $relRelPaiement["id_rel"]);
	...
	if ($id_lot != "" || $id_lot != null) {
		...
		foreach($impayes as $impaye) {
			if (intval($impaye["id_exercice"]) < 0) {
				...
				$codeHtml .= '<input type="checkbox" class="form-check-input" name="impayes[]" value="'.$impaye["id_rel"].'" '.$checked.'>Cumul des impayés N'.$impaye["id_exercice"];
				$codeHtml .= '</label>';
				$codeHtml .= '</div>';
				...
				$codeHtml .= '<div class="form-check form-check-inline">';
				$codeHtml .= '<label class="form-check-label">';
				$codeHtml .= '<input type="checkbox" class="form-check-input" name="impayes[]" value="'.$impaye["id_rel"].'" '.$checked.'>Impayé promoteur';
				$codeHtml .= '</label>';
				$codeHtml .= '</div>';
			}
		}
	...
	if (empty($error_msg) && isset($_POST['id'],$_POST['update'])) {
		...
		if ($id != '' && $update == 'true') {
			...
			$request = "UPDATE paiement SET id_lot=?, date=?, montant=?, id_modePaiement=?, commentaire=? WHERE id=?";
			if ($insert_stmt = $connection->prepare($request)) {
				$insert_stmt->bind_param('ssssss', $id_lot, $date, $montant, $id_modePaiement, $commentaire, $id);
				// Execute the prepared query.
				if (! $insert_stmt->execute()) {
					echo $connection->error;
					exit();
			...
			if(isset($_FILES['file']['name'])){
				if ($_FILES['file']['name'] != "") {
					...
					$location = __DIR__."/../justificatifs/paiements/".$id.".".$fileType;
					$oldFiles = glob("../justificatifs/paiements/".$id.".*");
					...
					$linkFile = getURL()."/../justificatifs/paiements/".$id.".".$fileType;
				}
			}
			$impayes = getImpaye($id_lot, null, $connection);
			foreach($impayes as $impaye){
				if($montant == 0) break;
				$tmpCotisation = floatval($impaye["partFonct"]) + floatval($impaye["partInv"]) - floatval($impaye["cotisation"]);
				$tmpCotisation =  $tmpCotisation < $montant ? $tmpCotisation : $montant;
				if(intval($impaye["id_exercice"]) <= 0) {
					if (in_array($impaye["id_rel"], $impayesAtraiter)) {
						...
						if ($insert_stmt = $connection->prepare($request)) {
							$insert_stmt->bind_param('ss', $cotisation, $impaye["id_rel"]);
							if (! $insert_stmt->execute()) {
								echo $connection->error;
								exit();
							} else {
								$request = "INSERT INTO rel_rel_paiement (id_rel, id_paiement, montant) VALUES (?, ?, ?)";
								if ($insert_stmt = $connection->prepare($request)) {
									$insert_stmt->bind_param('sss', $impaye["id_rel"], $id, $tmpCotisation);
									// Execute the prepared query.
									if (! $insert_stmt->execute()) {
										echo $connection->error;
										exit();
		...
		if ($insert_stmt = $connection->prepare($request)) {
            $insert_stmt->bind_param('ssssss', $id_lot, $date, $montant, $id_modePaiement, $commentaire, $id_syndic);
            // Execute the prepared query.
            if (! $insert_stmt->execute()) {
				echo $connection->error;
                exit();
		...
		if(isset($_FILES['file']['name'])){
			if ($_FILES['file']['name'] != "") {
				$fileType = pathinfo($_FILES['file']['name'],PATHINFO_EXTENSION);
				$fileType = strtolower($fileType);
				$location = __DIR__."/../justificatifs/paiements/".$insert_id.".".$fileType;
				move_uploaded_file($_FILES['file']['tmp_name'],$location);
				$linkFile = getURL()."/../justificatifs/paiements/".$insert_id.".".$fileType;
			}
		}
		$impayes = getImpaye($id_lot, null, $connection);
		foreach($impayes as $impaye){
				...
				if (in_array($impaye["id_rel"], $impayesAtraiter)) {
					...
					if ($insert_stmt = $connection->prepare($request)) {
						...
						if (! $insert_stmt->execute()) {
							...
							$request = "INSERT INTO rel_rel_paiement (id_rel, id_paiement, montant) VALUES (?, ?, ?)";
							if ($insert_stmt = $connection->prepare($request)) {
								$insert_stmt->bind_param('sss', $impaye["id_rel"], $insert_id, $tmpCotisation);
								// Execute the prepared query.
								if (! $insert_stmt->execute()) {
									echo $connection->error;
									exit();
...
if (isset($_GET['action'],$_GET['id'])):
	...
	$paiement = getPaiement($id, null, null, $connection);
	if (count($paiement) == 0 || ($_SESSION['id_usertype'] !== "1" && $_SESSION['id_usertype'] !== "2" && $_SESSION['id_usertype'] !== "3")) goto iomEnd;
		...
		<div class="content-body d-print-none">
			...
			<div class="container-fluid">
				<div class="form-head d-flex mb-3 align-items-start">
					...
					<button type="button" class="btn btn-rounded btn-primary px-3 my-1 me-2" id="windowPrint" data-id="<?=$paiement[0]["id"]?>">Imprimer</button>
				</div>
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                                    ...
                                    <form>
											...
											<div class="col-6 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_lot" id="id_lot" data-paiement="<?=$paiement[0]["id"]?>" data-action="update" class="single-select2 form-control wide mb-3" disabled>
                                                        <?php
                                                        $lots = getLot(null, null, $GLOBALS["id_copropriete"], $connection);
                                                        foreach($lots as $lot):
															$proprietaire = getProprietaire($lot["id_proprietaire"], null, $connection);
                                                        ?>
                                                        <option value="<?=$lot["id"]?>" <?php if($lot["id"] == $paiement[0]["id_lot"]) echo "selected"; ?>><?=$lot["code"]?> (<?=$proprietaire[0]["prenom"]?> <?=$proprietaire[0]["nom"]?>)</option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
                                            ...
                                            <div class="col-6 mb-2">
												<div class="form-group">
                                                    ...
                                                    <input type="date" class="form-control input-rounded input-default mb-3" name="date" placeholder="jj/mm/aaaa" value="<?=$paiement[0]["date"]?>">
                                                </div>
                                            </div>
											<div class="col-6 mb-2">
												<div class="form-group">
													<label class="text-label">Montant*</label>
													<input type="text" class="form-control input-rounded input-default mb-3" name="montant" placeholder="Montant" value="<?=$paiement[0]["montant"]?>">
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
                                            ...
                                            <div class="col-6 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_modePaiement" class="default-select form-control input-rounded wide mb-3">
                                                        ...
                                                        ?>
                                                        <option value="<?=$modepaiement["id"]?>" <?php if($modepaiement["id"] == $paiement[0]["id_modePaiement"]) echo "selected"; ?>><?=$modepaiement["libelle"]?></option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
											...
											<div class="col-6 mb-2">
												<div class="form-group">
                                                    ...
                                                    <textarea class="form-control input-rounded input-default mb-3" name="commentaire" placeholder="Commentaire"><?=$paiement[0]["commentaire"]?></textarea>
                                                </div>
                                            </div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<span id="lesImpayes">
														<?php
														$impayes = getImpaye($paiement[0]["id_lot"], $paiement[0]["id"], $connection);
														foreach($impayes as $impaye) :
															if (intval($impaye["id_exercice"]) < 0) :
														?>
														<div class="form-check form-check-inline">
															<label class="form-check-label">
																<input type="checkbox" class="form-check-input" name="impayes[]" value="<?=$impaye["id_rel"]?>" checked>Cumul des impayés N<?=$impaye["id_exercice"]?>
															</label>
														</div>
														<?php
															elseif (intval($impaye["id_exercice"]) == 0) :
														...
														<div class="form-check form-check-inline">
															<label class="form-check-label">
																<input type="checkbox" class="form-check-input" name="impayes[]" value="<?=$impaye["id_rel"]?>" checked>Impayé promoteur
															</label>
														</div>
														<?php
															endif;
				...
				<?php
				$preuves = glob ("./justificatifs/paiements/".$paiement[0]["id"].".*");
				?>
				<div class="row" id="blockOFjustificatif" <?php if(count($preuves) == 0) echo 'style="display: none;"'; ?>>
                    <div class="col-12">
                        <div class="card">
							...
							<div class="card-body">
								<div class="row">
									<div class="col-12">
										<?php 
										if(count($preuves) > 0) echo '<iframe src="'.$preuves[0]."?".uniqid().'" width="100%" height="500px"></iframe>'; 
										else echo '<iframe src="#" width="100%" height="500px"></iframe>';
										?>
									</div>
								</div>
							</div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="id_lot" id="id_lot" data-paiement="" data-action="add" class="single-select2 form-control wide mb-3">
                                                        ...
                                                        foreach($lots as $lot):
															$proprietaire = getProprietaire($lot["id_proprietaire"], null, $connection);
                                                        ?>
                                                        <option value="<?=$lot["id"]?>"><?=$lot["code"]?> (<?=$proprietaire[0]["prenom"]?> <?=$proprietaire[0]["nom"]?>)</option>
                                                        <?php
                                                        endforeach;
                                                        ?>
                                                    </select>
											...
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<span id="lesImpayes">
														...
														<div class="form-check form-check-inline">
															<label class="form-check-label">
																<input type="checkbox" class="form-check-input" name="impayes[]" value="<?=$impaye["id_rel"]?>">Cumul des impayés N<?=$impaye["id_exercice"]?>
															</label>
														</div>
														<?php
															elseif (intval($impaye["id_exercice"]) == 0) :
														...
														<div class="form-check form-check-inline">
															<label class="form-check-label">
																<input type="checkbox" class="form-check-input" name="impayes[]" value="<?=$impaye["id_rel"]?>">Impayé promoteur
															</label>
														</div>
														<?php
															endif;
...
<?php
	...
	iomEnd:
	$paiements = getPaiement(null, $GLOBALS["id_copropriete"], null, $connection);
?>
		<div class="content-body">
			...
			<div class="container-fluid">
				<div class="form-head d-flex mb-3 align-items-start">
					...
					?>
					<a href="export/export_paiement.php?id_copropriete=<?=$GLOBALS["id_copropriete"]?>&id_exercice=<?=$GLOBALS["id_exercice"]?>" type="button" class="btn btn-rounded btn-primary me-2">
                                ...
                                <div class="table-responsive">
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <?php
											...
											?>
                                            <tr class="trPaiement-<?=$paiement["id"]?>">
												...
												<td class="text-center">
													<a href="./dashboard.php?page=paiements&action=update&id=<?=$paiement["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
													<a href="javascript:void(0);" class="btn btn-secondary shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target=".delPaiement-<?=$paiement["id"]?>"><i class="fas fa-trash"></i></a>
												</td>
												<?php
												endif;
												?>
												...
												<div class="modal-dialog modal-dialog-centered" role="document">
													<div class="modal-content">
														...
														<div class="modal-footer">
															...
															<button type="button" class="btn btn-rounded btn-danger delPaiementBtn" data-id="<?=$paiement["id"]?>">Oui</button>
														</div>
													</div>
												</div>
											</div>
