...
{
    $request =
        "SELECT id, civilite, prenom, nom, email, telephone, mobile, adresse, ville, codePostale FROM proprietaire WHERE id NOT IN (SELECT id_proprietaire FROM lot)";
...
{
    $request =
        "SELECT id_rel FROM rel_lot_exercice WHERE id_lot = ? AND id_exercice = ?";
    if ($stmt = $connection->prepare($request)) {
        $stmt->bind_param("ss", $id_lot, $id_exercice);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($id_rel);
            while ($stmt->fetch()) {
                if (floatval($partFonct) == 0) {
                    $request = "DELETE FROM rel_lot_exercice WHERE id_rel = ?";
                } else {
                    $request =
                        "UPDATE rel_lot_exercice SET partFonct = ? WHERE id_rel = ?";
                }
            ...
            if (floatval($partFonct) > 0) {
                $request =
                    "INSERT INTO rel_lot_exercice (id_lot, id_exercice, partFonct, partInv, dateFinPeriode) VALUES (?, ?, ?, ?, ?)";
                if ($insert_stmt = $connection->prepare($request)) {
                    $impayeF = $partFonct;
                    $impayeI = 0;
                    ...
                    $dateFinAE = date(
                        ...
                        strtotime(
                            date(
                                "Y-m-d",
                                strtotime($copropriete[0]["dateExercice"]),
                            ) .
                                " " .
                                $id_exercice .
                                " year",
...
if (
    ...
    $dateAcquisition = filter_input(
        INPUT_POST,
        "dateAcquisition",
        FILTER_SANITIZE_STRING,
    );
    $dateRemiseCle = filter_input(
        INPUT_POST,
        "dateRemiseCle",
        FILTER_SANITIZE_STRING,
    );
    $id_proprietaire = filter_input(
        INPUT_POST,
        "id_proprietaire",
        FILTER_SANITIZE_STRING,
    );
    $id_typeProprietaire = filter_input(
        INPUT_POST,
        "id_typeProprietaire",
        FILTER_SANITIZE_STRING,
    );
    ...
    if (empty($error_msg) && isset($_POST["id"], $_POST["update"])) {
        ...
        if ($id != "" && $update == "true") {
            ...
            $request =
                "UPDATE lot SET code=?, password=?, id_typeLot=?, numeroImm=?, etage=?, numero=?, foncier=?, tantieme=?, dateAcquisition=?, dateRemiseCle=?, id_proprietaire=?, id_typeProprietaire=?, impaye0=?, impaye1=?, impaye2=?, impaye3=?, impaye4=?, impaye5=? WHERE id=?";
...
if (isset($_GET["action"], $_GET["id"])):
		...
		<div class="content-body">
			...
			<div class="container-fluid">
              ...
              foreach ($typelots as $typelot): ?>
														<option value="<?= $typelot["id"] ?>" <?php if (
    $typelot["id"] == $lot[0]["id_typeLot"]
) {
    echo "selected";
} ?>><?= $typelot["libelle"] ?></option>
														<?php endforeach;
              ?>
													</select>
												</div>
											...
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="numeroImm" placeholder="Numéro d'immeuble" value="<?= $lot[0][
                 "numeroImm"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="etage" placeholder="Numéro d'étage" value="<?= $lot[0][
                 "etage"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="numero" placeholder="Numéro du lot" value="<?= $lot[0][
                 "numero"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="foncier" placeholder="Titre foncier" value="<?= $lot[0][
                 "foncier"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="tantieme" placeholder="Tantième" value="<?= floatval(
                 $lot[0]["tantieme"],
             ) ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="dateAcquisition" placeholder="Date d'acquisition"  value="<?= $lot[0][
                 "dateAcquisition"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="dateRemiseCle" placeholder="Date de remise des clés" value="<?= $lot[0][
                 "dateRemiseCle"
             ] ?>" <?= $disabled ?>>
												</div>
											</div>
              ...
              foreach ($proprietaires as $proprietaire): ?>
														<option value="<?= $proprietaire["id"] ?>" <?php if (
    $proprietaire["id"] == $lot[0]["id_proprietaire"]
) {
    echo "selected";
    ...
    echo "selected";
              ...
              foreach ($typeproprietaires as $typeproprietaire): ?>
														<option value="<?= $typeproprietaire["id"] ?>" <?php if (
    $typeproprietaire["id"] == $lot[0]["id_typeProprietaire"]
) {
    echo "selected";
} ?>><?= $typeproprietaire["libelle"] ?></option>
														<?php endforeach;
              ?>
													</select>
												</div>
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye1" placeholder="0.00" value="<?= $lot[0][
                 "impaye1"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye2" placeholder="0.00" value="<?= $lot[0][
                 "impaye2"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye3" placeholder="0.00" value="<?= $lot[0][
                 "impaye3"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye4" placeholder="0.00" value="<?= $lot[0][
                 "impaye4"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye5" placeholder="0.00" value="<?= $lot[0][
                 "impaye5"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
             ...
             ?>
													<input type="text" class="form-control input-rounded input-default mb-3" name="impaye0" placeholder="0.00" value="<?= $lot[0][
                 "impaye0"
             ] ?>" <?= $disabled ?>>
													<?php $disabled = $tempdisabled; ?>
												</div>
                                        ...
                                        <div class="row">
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="text" class="form-control input-rounded input-default mb-3" name="identifiant" placeholder="Identifiant" value="<?= $lot[0][
                 "code"
             ] ?>" disabled>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													...
													<input type="password" class="form-control input-rounded input-default mb-3" name="password" id="passInput" placeholder="Mot de passe" value="<?= $lot[0][
                 "password"
             ] ?>">
												</div>
											</div>
    ...
    elseif ($action == "view" && $id != ""):
		...
		<div class="content-body d-print-none">
          ...
          "code"
					...
					<a href="export/export_proprietaire.php?id=<?= $lot[0][
         "id"
     ] ?>&id_exercice=<?= $GLOBALS[
    "id_exercice"
     ...
     "nom"
              ...
              echo "N/A";
										...
										<p class="font-w600 mb-2 d-flex"><span class="custom-label-2">Date d'acquisition :</span> <span class="font-w400"><?php if (
              $lot[0]["dateAcquisition"] != null &&
              $lot[0]["dateAcquisition"] != ""
          ) {
              echo date("d/m/Y", strtotime($lot[0]["dateAcquisition"]));
          } ?></span></p>
										<p class="font-w600 mb-2 d-flex"><span class="custom-label-2">Date de remise des clés : </span><span class="font-w400"><?php if (
              $lot[0]["dateRemiseCle"] != null &&
              $lot[0]["dateRemiseCle"] != ""
          ) {
              echo date("d/m/Y", strtotime($lot[0]["dateRemiseCle"]));
          } ?></span></p>
									</div>
								</div>
							</div>
         ...
         if ($totalPayeChecker >= $totalImpayeChecker) {
             echo '<span class="d-block mb-1"><i class="fas fa-circle text-success me-2"></i>Ce propriétaire est <strong>créditeur</strong></span>';
         } else {
             echo '<span class="d-block mb-1"><i class="fas fa-circle text-danger me-2"></i>Ce propriétaire est <strong>débiteur</strong></span>';
         }
         ?>
								</div>
							</div>
            ...
            ?>
                                        ...
                                        <tbody>
                                            <tr>
                                                <td rowspan="2"><strong><?= getNameexercice(
            ...
            endforeach;
            ?>
                                                <td rowspan="2"><strong><?= number_format(
                                                    $totalImpaye,
                                                    2,
                                                ) ?></strong></td>
                                            </tr>
    ...
    ?>
                 ...
                 ?>
									<div class="col-2">
										<div class="bgl-<?= $bgl ?> rounded p-3">
											<h6 class="mb-2">
												<?php if (intval($periode["id_exercice"]) < 0): ?>
												Cumul des impayés N<?= $periode["id_exercice"] ?>
												<?php elseif (intval($periode["id_exercice"]) == 0): ?>
												Impayé promoteur
												<?php endif; ?>
											</h6>
     ...
     "prenom"
                                                ...
                                                <td><?= $syndic[0]["civilite"] .
                                                    ...
                                                    $syndic[0]["nom"] ?></td>
												<td class="text-center">
													<button class="btn btn-primary shadow btn-xs sharp me-1"  data-bs-toggle="modal" data-bs-target="#paiement-<?= $paiement[
                 "id"
             ] ?>"><i class="fas fa-eye"></i></button>
													<div class="modal fade" id="paiement-<?= $paiement[
                 "id"
             ] ?>" tabindex="-1" style="display: none;" aria-hidden="true">
														<div class="modal-dialog modal-lg">
															<div class="modal-content">
																<div class="modal-header">
																	<h5 class="modal-title"><strong>Reçu de paiement N° : <?= date("y") .
                     sprintf("%'.05d", $paiement["id"]) ?></strong></h5>
																...
																<div class="modal-body">
																	<div class="row mb-0">
																		...
																		<div class="mt-2 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 text-start">
																			<div><strong><?= $proprietaire[0]["civilite"] ?> <?= $proprietaire[0][
     "prenom"
 ] ?> <?= $proprietaire[0]["nom"] ?></strong></div>
																	...
																	<div class="row">
																		<div class="col-xl-12 col-md-12">
																			<?php $modepaiement = getModepaiement(
                       $paiement["id_modePaiement"],
                       $connection,
                   ); ?>
																			<p class="font-w400 mb-2 text-start"><span class="custom-label-2">Montant total : </span><span class="font-w600"> <?= number_format(
                       floatval($paiement["montant"]),
                       2,
                       ",",
                       " ",
                   ) ?> MAD</span></p>
																			<p class="font-w400 mb-2 text-start"><span class="custom-label-2">Mode de paiement : </span><span class="font-w600"> <?= $modepaiement[0][
                       "libelle"
                   ] ?></span></p>
                       ...
                       foreach ($relRelPaiements as $relRelPaiement) {
                           ...
                           echo '<td class="text-center font-w400 p-2"><strong>' .
                               $periodeInfo[0]["nomPeriode"] .
                               "</strong></td>";
                           $TotalRelPaiement += floatval(
                       ...
                       ?>
																						...
																						<tr>
																							<?php
                       foreach ($relRelPaiements as $relRelPaiement) {
                           echo '<td class="text-center font-w400 p-2">' .
                               number_format(
    ...
    if ($action == "add"): ?>
		<div class="content-body">
			...
			<div class="container-fluid">
              ...
              foreach ($proprietaires as $proprietaire): ?>
														<option value="<?= $proprietaire["id"] ?>"><?= $proprietaire[
    "prenom"
...
<?php else:goto iomEnd;endif;
		...
		<div class="content-body d-print-none">
         ...
         $currentPeriodePaiement =
             count($exportExercice) > 0
                 ...
                 : "1";
         $periodePaiementOptions = [
             "1" => "Mensuel",
             "2" => "Trimestriel",
             "3" => "Semestriel",
             "4" => "Annuel",
         ];
         if (!isset($periodePaiementOptions[$currentPeriodePaiement])) {
             $currentPeriodePaiement = "1";
         }
         $orderedPeriodePaiementOptions =
    ...
    "id_exercice"
     ...
     "prenom"
                                                ...
                                                <td class="text-center">
													<a href="./dashboard.php?page=contentieux&action=update&id=<?= $lot[
                 "id"
             ] ?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-balance-scale"></i></a>
												</td>
                                                <td class="text-center">
													<a href="./dashboard.php?page=lots&action=view&id=<?= $lot[
                 "id"
             ] ?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>
													<a href="./dashboard.php?page=lots&action=update&id=<?= $lot[
                 "id"
