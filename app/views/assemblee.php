...
if (isset(
    ...
    $ordreDuJour = filter_input(INPUT_POST, 'ordreDuJour', FILTER_SANITIZE_STRING);
	...
	if (empty($error_msg) && isset($_POST['update'])) {
		...
		if ($update == 'true') {
			...
			if (count($assemblee) > 0) {
				$id = $assemblee[0]["id"];
					$request = "UPDATE assemblee SET date1=?, date2=?, date3=?, objet=?, ordreDuJour=?, titre1=?, titre2=? WHERE id=?";

					if ($insert_stmt = $connection->prepare($request)) {
						$insert_stmt->bind_param('ssssssss', $date1, $date2, $date3, $objet, $ordreDuJour, $titre1, $titre2, $id);
				...
				if ($insert_stmt = $connection->prepare($request)) {
					$insert_stmt->bind_param('ssssssss', $date1, $date2, $date3, $objet, $ordreDuJour, $titre1, $titre2, $id_exercice);
					// Execute the prepared query.
					if (! $insert_stmt->execute()) {
						echo $connection->error;
						exit();
			...
			<div class="container-fluid">
				<div class="form-head d-flex mb-3 align-items-start">
					<div class="me-auto d-none d-lg-block">
						<h2 class="text-primary font-w600 mb-0">Assemblée générale (<?=getNameexercice($exercice[0]["dateDebut"])?>)</h2>
                                        ...
                                        <div class="row">
											<div class="col-4 mb-2">
												<div class="form-group">
													...
													<input type="datetime-local" class="form-control input-rounded input-default mb-3" name="date1" placeholder="Date 1" value="<?=count($assemblee)?$assemblee[0]["date1"]:""?>">
												</div>
											</div>
											<div class="col-4 mb-2">
												<div class="form-group">
													<label class="text-label">Date de la 2<sup>ème</sup> séance*</label>
													<input type="datetime-local" class="form-control input-rounded input-default mb-3" name="date2" placeholder="Date 2" value="<?=count($assemblee)?$assemblee[0]["date2"]:""?>">
												</div>
											</div>
											<div class="col-4 mb-2">
												<div class="form-group">
													<label class="text-label">Date de consultation des documents*</label>
													<input type="datetime-local" class="form-control input-rounded input-default mb-3" name="date3" placeholder="Date 3" value="<?=count($assemblee)?$assemblee[0]["date3"]:""?>">
												</div>
											</div>
											<div class="col-12 mb-2">
												<div class="form-group">
													<label class="text-label">Objet*</label>
													<input type="text" class="form-control input-rounded input-default mb-3" name="objet" placeholder="Objet" value="<?=count($assemblee)?$assemblee[0]["objet"]:""?>">
												</div>
											</div>
											<div class="col-12 mb-4">
												<div class="form-group">
													<label class="text-label">Ordre du jour*</label>
													<textarea name="ordreDuJour" id="ordreDuJour" cols="30" rows="5" class="form-control input-rounded" placeholder="Ordre du jour ..."><?=count($assemblee)?$assemblee[0]["ordreDuJour"]:""?></textarea>
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													<label class="text-label">Titre de la feuille de présence à l'assemblée générale</label>
													<input type="text" class="form-control input-rounded input-default mb-3" name="titre1" placeholder="Titre de la feuille" value="<?=count($assemblee)?$assemblee[0]["titre1"]:""?>">
												</div>
											</div>
											<div class="col-6 mb-2">
												<div class="form-group">
													<label class="text-label">Titre de la liste de réception des documents</label>
													<input type="text" class="form-control input-rounded input-default mb-3" name="titre2" placeholder="Titre de la liste" value="<?=count($assemblee)?$assemblee[0]["titre2"]:""?>">
												</div>
											</div>
										</div>
                                    </form>
