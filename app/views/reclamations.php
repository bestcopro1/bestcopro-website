...
if (isset($_POST['id_reclamation'], $_POST['id_statutReclamation'], $_POST['commentaire'])) {
	
	if (!isset($_SESSION['id']) || (isset($_SESSION['id']) && !is_int(intval($_SESSION['id'])))) {
		echo "Une erreur est survenue !";
		exit();
	}
	
	$id_reclamation = filter_input(INPUT_POST, 'id_reclamation', FILTER_SANITIZE_STRING);
	$id_statutReclamation = filter_input(INPUT_POST, 'id_statutReclamation', FILTER_SANITIZE_STRING);
	...
	if ($commentaire != "") {
		...
		if ($insert_stmt = $connection->prepare($request)) {
			$insert_stmt->bind_param('ssss', $date, $_SESSION['id'], $commentaire, $id_reclamation);
	...
	if ($id_statutReclamation == "2") 
		$date = date('Y-m-d H:i:s');
	$request = "UPDATE reclamation SET id_statutReclamation=?, dateFermeture=? WHERE id=?";
	if ($insert_stmt = $connection->prepare($request)) {
		$insert_stmt->bind_param('sss', $id_statutReclamation, $date, $id_reclamation);
		// Execute the prepared query.
		if (! $insert_stmt->execute()) {
			echo $connection->error;
			exit();
	...
	if ($commentaire != "") {
		...
		echo "done|".date('d/m/Y')."|".$syndic[0]["civilite"]." ".$syndic[0]["prenom"]." ".$syndic[0]["nom"]."|".$commentaire;
	} else
		echo "done|X";
	exit();
}
if (isset($_GET['action'],$_GET['id'])):
	...
	if (count($reclamation) == 0) goto iomEnd;
	$date = date("d/m/Y", strtotime($reclamation[0]["date"]));
	if($action == "update" && $id != ""):
		$request = "UPDATE notificationsyndic SET seen = 1 WHERE idPage = ? AND nomPage = 'reclamations'"; 
		if ($insert_stmt = $connection->prepare($request)) {
                                                ...
                                                <div class="read-content">
                                                    <div class="media pt-3 d-sm-flex d-block justify-content-between">
														<div class="clearfix mb-3 d-flex">
															<?php
															$preuves = glob ("./justificatifs/reclamations/".$reclamation[0]["id"].".*");
															if(count($preuves) == 0) :
															?>
														...
														<div class="clearfix mb-3">
															...
															<select name="id_statutReclamation" class="default-select form-control input-rounded wide mb-3">
																...
																?>
																<option value="<?=$statutReclamation["id"]?>" <?php if($statutReclamation["id"] == $reclamation[0]["id_statutReclamation"]) echo "selected"; ?>><?=$statutReclamation["libelle"]?></option>
																<?php
																endforeach;
																?>
															</select>
                                                    ...
                                                    <div class="media mb-2 mt-3">
                                                        <div class="media-body"><span class="pull-end"><?=date("d/m/Y", strtotime($message["date"]))?></span>
															<?php
															if ($message["id_syndic"] != null) :
																$syndic = getSyndic($message["id_syndic"], null, $connection);
															?>
                                                            <h5 class="my-1 text-primary"><?=$syndic[0]["civilite"]?> <?=$syndic[0]["prenom"]?> <?=$syndic[0]["nom"]?></h5>
															<?php
															else :
																$proprietaire = getProprietaire($message["id_proprietaire"], null, $connection);
															?>
															<h5 class="my-1 text-secondary"><?=$proprietaire[0]["civilite"]?> <?=$proprietaire[0]["prenom"]?> <?=$proprietaire[0]["nom"]?></h5>
															<?php
															endif;
															?>
                                                        </div>
													...
													<div class="mb-3 pt-3">
                                                        <textarea name="commentaire" id="commentaire" cols="30" rows="5" class="form-control input-rounded" placeholder="Votre réponse ..." <?php if ($reclamation[0]["id_statutReclamation"] == "2") echo 'style="display: none;"'; ?>></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<?php
	...
	iomEnd:
	$reclamations = getReclamation(null, null, $GLOBALS["id_copropriete"], $GLOBALS["connection"]);
?>
		<div class="content-body">
            <!-- row -->
			<div class="container-fluid">
				<div class="form-head d-flex mb-3 align-items-start">
					...
					?>
					<a href="export/export_reclamation.php?id_copropriete=<?=$GLOBALS["id_copropriete"]?>&id_exercice=<?=$GLOBALS["id_exercice"]?>" type="button" class="btn btn-rounded btn-primary me-2">
                                    ...
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <?php
											foreach($reclamations as $reclamation):
												$lot = getLot($reclamation["id_lot"], null, $GLOBALS["id_copropriete"], $connection);
											...
											?>
                                            <tr>
                                                <td><?=date("d/m/Y", strtotime($reclamation["date"]))?></td>
                                                <td><?=$proprietaire[0]["civilite"]?> <?=$proprietaire[0]["prenom"]?> <?=$proprietaire[0]["nom"]?></td>
                                                <td><?=$reclamation["objet"]?></td>
                                                <td>
													<?php
													if ($reclamation["id_statutReclamation"] == "1") 
														echo '<span class="badge badge-rounded badge-danger">'.$statutReclamation[0]["libelle"].'</span>';
													elseif($reclamation["id_statutReclamation"] == "2") 
														echo '<span class="badge badge-rounded badge-success">'.$statutReclamation[0]["libelle"].'</span>';
													?>
												</td>
                                                <td>
													<?php
													if ($reclamation["dateFermeture"] != null) 
                                                ...
                                                <td class="text-center">
													<a href="./dashboard.php?page=reclamations&action=update&id=<?=$reclamation["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>
												</td>
                                            </tr>
                                            <?php
											endforeach;
