...
if (isset(
    ...
    if (empty($error_msg) && isset($_POST['id'],$_POST['update'])) {
		...
		if ($id != '' && $update == 'true') {
			...
			$request = "UPDATE copropriete SET nom=?, ville=?, adresse=?, codePostale=?, rib=?, id_syndic=?, prefixe=? WHERE id=?";

			if ($insert_stmt = $connection->prepare($request)) {
				$insert_stmt->bind_param('ssssssss', $nom, $ville, $adresse, $codePostale, $rib, $id_syndic, $prefixe, $id);
				// Execute the prepared query.
				if (! $insert_stmt->execute()) {
					echo $connection->error;
					exit();
                                        ...
                                        <div class="row">
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													...
													<input type="text" name="nom" class="form-control input-rounded input-default mb-3" placeholder="Nom de la copropriété" value="<?=$GLOBALS["copropriete"][0]["nom"]?>" required="" <?=$disabled?>>
												</div>
											</div>
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">Ville*</label>
													<input type="text" name="ville" class="form-control input-rounded input-default mb-3" placeholder="Ville" value="<?=$GLOBALS["copropriete"][0]["ville"]?>" required="" <?=$disabled?>>
												</div>
											</div>
											<div class="col-lg-12 mb-2">
												<div class="form-group">
													<label class="text-label">Adresse*</label>
													<input type="text" name="adresse" class="form-control input-rounded input-default mb-3" placeholder="Adresse de la copropriété" value="<?=$GLOBALS["copropriete"][0]["adresse"]?>" required="" <?=$disabled?>>
												</div>
											</div>
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">Code postale</label>
													<input type="text" name="codePostale" class="form-control input-rounded input-default mb-3" placeholder="Code postale" value="<?=$GLOBALS["copropriete"][0]["codePostale"]?>" required="" <?=$disabled?>>
												</div>
											</div>
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">RIB</label>
													<input type="text" name="rib" class="form-control input-rounded input-default mb-3" placeholder="RIB" value="<?=$GLOBALS["copropriete"][0]["rib"]?>" required="" <?=$disabled?>>
												</div>
											</div>
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">Nombre de lots*</label>
													<input type="number" min="1" name="nbrLot" class="form-control input-rounded input-default mb-3" placeholder="Nombre de lots" value="<?=$GLOBALS["copropriete"][0]["nbrLot"]?>" required="" disabled>
												</div>
											</div>
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">Date d'ouverture de l'exercice*</label>
													<input type="month" name="dateExercice" class="form-control input-rounded input-default mb-3" placeholder="mm/yyyy" value="<?=date("Y-m",strtotime($GLOBALS["copropriete"][0]["dateExercice"]))?>" required="" disabled>
												</div>
											</div>
											<input type="hidden" name="id_syndic" value="<?=$GLOBALS["copropriete"][0]["id_syndic"]?>">
											<div class="col-lg-6 mb-2">
												<div class="form-group">
													<label class="text-label">Préfixe*</label>
													<input type="text" name="prefixe" class="form-control input-rounded" placeholder="Préfixe des références" value="<?=$GLOBALS["copropriete"][0]["prefixe"]?>" required="" <?=$disabled?>>
												</div>
											</div>
										</div>
									</form>
                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button type="button" class="btn btn-rounded btn-danger" id="deleteCoproprieteBtn" data-id="<?=$GLOBALS["copropriete"][0]["id"]?>">Oui</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
