...
if (isset($_POST['rubrique2_1'], $_POST['id_exercice'])) {
	...
	if ($id_exercice != "") {
		...
		if(number_format(floatval($montantTotal)) != number_format(floatval($exercice[0]["montantInvest"]))) {
			$error_msg .= 'Le budget annuel de fonctionnement doit être égal à '.$exercice[0]["montantInvest"].' MAD';
			echo $error_msg;
			exit();
		}
		$id_typeRubrique = 2;
		...
		while (isset($_POST['rubrique2_'.$i])) {
			...
			if ($rubrique != "") {
				if (isset($_POST['rubrique2_'.$i.'_id'])) {
					$id_rubrique = filter_input(INPUT_POST, 'rubrique2_'.$i.'_id', FILTER_SANITIZE_STRING);
				...
				while (isset($_POST['rubrique2_'.$i.'_poste2_'.$j])) {
					...
					if ($poste != "" && $poste_value > 0) {
						if (isset($_POST['rubrique2_'.$i.'_poste2_'.$j.'_id'])) {
							$id_poste = filter_input(INPUT_POST, 'rubrique2_'.$i.'_poste2_'.$j.'_id', FILTER_SANITIZE_STRING);
...
?>
		<div class="content-body">
			...
			<div class="container-fluid">
				...
				<input type="hidden" name="montantTotal" id="montantTotal" value="<?=$exercice[0]["montantInvest"]?>">
				<div class="alert alert-primary alert-alt fade show p-3 mb-4">
					<div class="row">
						<div class="col-lg-6">
							<strong>Budget annuel d'investissement de l'<?=getNameexercice($exercice[0]["dateDebut"])?></strong>
						</div>
						<div class="col-lg-6 text-end">
							<strong>TOTAL = <span id="totalBudget" class="font-w500"><?=$exercice[0]["montantInvest"]?></span> MAD</strong>
						</div>
					</div>
				</div>
                <div class="row">
								...
								<div class="basic-list-group rubrique2_<?=$i?>">
									<ul class="list-group">
										<li class="list-group-item active">
											...
											<div class="row">
												<div class="col-12">
													<input type="hidden" name="rubrique2_<?=$i?>_id" value="<?=$rubrique["id"]?>">
											...
											<div class="col-12">
													<input type="hidden" name="rubrique2_<?=$i?>_id" value="<?=$rubrique["id"]?>">
													<input type="text" class="form-control input-rounded" name="rubrique2_<?=$i?>" placeholder="Nouvelle rubrique" value="<?=$rubrique["libelle"]?>" disabled>
												</div>
											<?php
											endif;
											?>
										...
										if(count($postes) > 0):
											...
											<li class="list-group-item rubrique2_<?=$i?>_poste2_<?=$j?>">
												<div class="row">
													<div class="col-6">
														<input type="hidden" name="rubrique2_<?=$i?>_poste2_<?=$j?>_id" value="<?=$poste["id"]?>">
														<input type="text" class="form-control input-rounded" name="rubrique2_<?=$i?>_poste2_<?=$j?>" placeholder="Nouveau poste" value="<?=$poste["libelle"]?>" <?=$disabled?>>
													</div>
													<?php
													if ($_SESSION['id_usertype'] === "1") :
													...
													?>
													<div class="col-6">
														<input type="number" class="form-control input-rounded value" name="rubrique2_<?=$i?>_poste2_<?=$j?>_value" placeholder="0.00" value="<?=$poste["montant"]?>">
													</div>
													<?php
														else :
													?>
