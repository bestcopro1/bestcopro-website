...
if (isset(
    ...
    $dateContentieux = filter_input(INPUT_POST, 'dateContentieux', FILTER_SANITIZE_STRING);
	...
	if (empty($error_msg)) {
		...
		if ($insert_stmt = $connection->prepare($request)) {
            $insert_stmt->bind_param('ssss', $id_lot, $dateContentieux, $id_etat, $remarque);
            // Execute the prepared query.
            if (! $insert_stmt->execute()) {
				echo $connection->error;
                exit();
		...
		if ($id_etat == "1") {
			...
			$description = "Rappel : Contentieux pour le lot ".$lot[0]["code"];
		...
		$codeHtml .= 	'<span>'.date("d/m/Y", strtotime($dateContentieux)).'</span>';
...
?>
		<div class="content-body">
			...
			<div class="container-fluid">
							...
							<div class="card-body">
								<div class="row">
									...
									<div class="col-xl-6 col-md-6">
										...
										<p class="font-w600 mb-2 d-flex"><span class="custom-label-2">Date d'acquisition :</span> <span class="font-w400"><?=date("d/m/Y", strtotime($lot[0]["dateAcquisition"]))?></span></p>
										<p class="font-w600 mb-2 d-flex"><span class="custom-label-2">Date de remise des clés : </span><span class="font-w400"><?=date("d/m/Y", strtotime($lot[0]["dateRemiseCle"]))?></span></p>
									</div>
								</div>
							</div>
						</div>
				...
				<?php
				$style = '';
				if (count($contentieuxs) > 0) $style = ''; else $style = 'style="display: none;"';
				?>
								...
								<div id="DZ_W_TimeLine" class="widget-timeline dlab-scroll px-4">
                                    <ul class="timeline">
										...
										foreach($contentieuxs as $contentieux):
											...
											if ($contentieux["id_etat"] == "1")
												$badgeColor = "warning";
											elseif($contentieux["id_etat"] == "2")
												$badgeColor = "danger";
											elseif($contentieux["id_etat"] == "3")
												$badgeColor = "dark";
											elseif($contentieux["id_etat"] == "4")
												$badgeColor = "info";
											elseif($contentieux["id_etat"] == "5")
												$badgeColor = "success";
										?>
										<li>
                                            <div class="timeline-badge <?=$badgeColor?>"></div>
                                            <div class="timeline-panel">
                                                <span><?=date("d/m/Y", strtotime($contentieux["date"]))?></span>
                                                <h6 class="mb-0"><?=$etat[0]["libelle"]?></h6>
...
<?php
	...
	iomEnd:
	$contentieuxs = getContentieux(null, $GLOBALS["id_copropriete"], $connection);
?>
		<div class="content-body">
            <!-- row -->
			<div class="container-fluid">
                                    ...
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <tbody>
											...
											foreach($contentieuxs as $contentieux):
												...
												$etat = getEtat($contentieux["id_etat"], $connection);
											?>
                                            <tr class="trContentieux-<?=$lot[0]["id"]?>">
                                                <td><?=$lot[0]["code"]?></td>
                                                <td><?=$proprietaire[0]["civilite"]?> <?=$proprietaire[0]["prenom"]?> <?=$proprietaire[0]["nom"]?></td>
												<td><?=$etat[0]["libelle"]?></td>
                                                <td><?=date("d/m/Y", strtotime($contentieux["date"]))?></td>
                                                <td class="text-center">
													<?php
													if ($_SESSION['id_usertype'] === "1" || $_SESSION['id_usertype'] === "2" || $_SESSION['id_usertype'] === "3") :
													?>
													<a href="./dashboard.php?page=contentieux&action=update&id=<?=$lot[0]["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
													<?php
													else :
													?>
													<a href="./dashboard.php?page=contentieux&action=update&id=<?=$lot[0]["id"]?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>
													<?php
													endif;
													?>
													<?php
													...
													?>
													<a href="javascript:void(0);" class="btn btn-secondary shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target=".delContentieux-<?=$lot[0]["id"]?>"><i class="fas fa-trash"></i></a>
													<?php
													endif;
													?>
												</td>
												...
												<div class="modal-dialog modal-dialog-centered" role="document">
													<div class="modal-content">
														...
														<div class="modal-footer">
															...
															<button type="button" class="btn btn-rounded btn-danger delContentieuxBtn" data-id="<?=$lot[0]["id"]?>">Oui</button>
														</div>
													</div>
												</div>
											</div>
