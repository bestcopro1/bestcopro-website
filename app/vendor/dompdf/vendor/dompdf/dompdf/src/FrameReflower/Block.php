...
class Block extends AbstractFrameReflower
{
    ...
    protected function _calculate_width($width)
    {
        ...
        if ($absolute) {
            ...
            if ($width === "auto" || $left === "auto" || $right === "auto") {
                ...
                $inflow_x =
                    $parent_content["x"] - $cb["x"] + $line->left + $line->w;
    ...
    protected function _calculate_restricted_width()
    {
        ...
        if (!isset($cb["w"])) {
            throw new Exception(
                "Box property calculation requires containing block width",
            );
        }
    ...
    protected function _calculate_restricted_height()
    {
        ...
        if ($frame->is_absolute()) {
            ...
            $sum =
                (float) $style->length_in_pt($h_dims, $cb["h"]) +
    ...
    protected function _text_align()
    {
        ...
        switch ($style->text_align) {
            ...
            case "justify":
                ...
                // the split-off frame
                $lines = $this->_frame->get_line_boxes();
                $last_line_index = $this->_frame->is_split
                    ? null
                    : count($lines) - 1;
    ...
    function vertical_align()
    {
        ...
        foreach ($this->_frame->get_line_boxes() as $line) {
            ...
            foreach ($line->frames_to_align() as $frame) {
                ...
                $baseline = $fontMetrics->getFontBaseline(
                    $style->font_family,
                    $style->font_size,
                );
                $y_offset = 0;
                ...
                if ($isInlineBlock) {
                    ...
                    if (
                        in_array($align, Style::VERTICAL_ALIGN_KEYWORDS, true)
                    ) {
                        switch ($align) {
                            case "middle":
    ...
    function process_clear(AbstractFrameDecorator $child)
    {
        ...
        if ($child_style->clear !== "none") {
            ...
            if ($child_style->float !== "none" && $child->get_next_sibling()) {
                $this->_frame->set_current_line_number(
                    $this->_frame->get_current_line_number() - 1,
                );
            }
    ...
    function reflow(BlockFrameDecorator $block = null)
    {
        ...
        [
            ...
            $left,
            $right,
        ] = $this->_calculate_restricted_width();
        ...
        [
            ...
            $top,
            $bottom,
        ] = $this->_calculate_restricted_height();
