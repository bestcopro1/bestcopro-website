...
class Cache
{
        ...
        try {
            ...
            if (!$is_data_uri) {
                ...
                if (!array_key_exists($protocol, $allowed_protocols)) {
                    throw new ImageException(
                        "Permission denied on $url. The communication protocol is not supported.",
                        E_WARNING,
                    );
                }
            ...
            if ($protocol === "file://") {
                ...
                if (
                    ...
                    throw new ImageException(
                        "Unable to create temporary image in " . $tmp_dir,
                        E_WARNING,
                    );
                }
                ...
                if ($is_data_uri) {
                    if (
                        ($parsed_data_uri = Helpers::parse_data_uri($url)) !==
                        false
                    ) {
                        $image = $parsed_data_uri["data"];
                    }
                } else {
                    [$image, $http_response_header] = Helpers::getFileContent(
                        $full_url,
                        $options->getHttpContext(),
                    );
                }
                ...
                xml_set_element_handler(
                    ...
                    function ($parser, $name, $attributes) use (
                        ...
                        if (strtolower($name) === "image") {
                            ...
                            foreach ($urls as $url) {
                                if (!empty($url)) {
                                    $inner_full_url = Helpers::build_url(
                                        $parsed_url["protocol"],
                                        $parsed_url["host"],
                                        $parsed_url["path"],
                                        $url,
                                    ...
                                    if ($inner_full_url === $full_url) {
                                        throw new ImageException(
                                            "SVG self-reference is not allowed",
                                            E_WARNING,
                                        );
                                    }
                                    [
                                    ...
                                    if (!empty($message)) {
                                        throw new ImageException(
                                            "This SVG document references a restricted resource. $message",
                                            E_WARNING,
                                        );
                                    }
                                }
    ...
    {
        [, , $type] = Helpers::dompdf_getimagesize($file, $context);
...
if (file_exists(realpath(__DIR__ . "/../../lib/res/broken_image.svg"))) {
    Cache::$broken_image = realpath(
        __DIR__ . "/../../lib/res/broken_image.svg",
    );
}
