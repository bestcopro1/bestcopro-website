...
class CPdf
{
    ...
    private function openFont($font)
    {
        ...
        if (file_exists($fontcache . "/" . $cache_name)) {
            $this->addMessage(
                "openFont: php file exists $fontcache/$cache_name",
            );
            $this->fonts[$font] = require $fontcache . "/" . $cache_name;

            if (
                !isset($this->fonts[$font]["_version_"]) ||
                $this->fonts[$font]["_version_"] != $this->fontcacheVersion
            ) {
            ...
            if (file_exists($fontcache . "/" . $old_cache_name)) {
                ...
                $old_cache = file_get_contents(
                    $fontcache . "/" . $old_cache_name,
                );
                file_put_contents(
                    $fontcache . "/" . $cache_name,
                    "<?php return " . $old_cache . ";",
                );
