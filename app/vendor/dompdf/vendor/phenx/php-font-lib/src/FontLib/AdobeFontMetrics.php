...
class AdobeFontMetrics
{
    ...
    function write($file, $encoding = null)
    {
        ...
        $this->addPair(
            "Weight",
            $os2["usWeightClass"] > 400 ? "Bold" : "Medium",
        );
        ...
        $this->addPair(
            "UnderlineThickness",
            $font->normalizeFUnit($post["underlineThickness"]),
        );
        $this->addPair(
            "UnderlinePosition",
            $font->normalizeFUnit($post["underlinePosition"]),
        );
        ...
        if (isset($hhea["ascent"])) {
            $this->addPair(
                "FontHeightOffset",
                $font->normalizeFUnit($hhea["lineGap"]),
            );
            ...
            $this->addPair(
                "FontHeightOffset",
                $font->normalizeFUnit($os2["typoLineGap"]),
            );
            $this->addPair(
                "Ascender",
                $font->normalizeFUnit($os2["typoAscender"]),
            );
            $this->addPair(
                "Descender",
                -abs($font->normalizeFUnit($os2["typoDescender"])),
            );
        }
        ...
        if ($glyphIndexArray) {
            ...
            if (!$encoding && is_array($tree)) {
                ...
                $this->startSection(
                    "KernPairs",
                    count($tree, COUNT_RECURSIVE) - count($tree),
                );
