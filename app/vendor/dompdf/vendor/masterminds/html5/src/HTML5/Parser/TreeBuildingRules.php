...
class TreeBuildingRules
{
    ...
    public function evaluate($new, $current)
    {
        switch ($new->tagName) {
            ...
            case "option":
                return $this->closeIfCurrentMatches($new, $current, ["option"]);
            case "tr":
                return $this->closeIfCurrentMatches($new, $current, ["tr"]);
            case "td":
            case "th":
                return $this->closeIfCurrentMatches($new, $current, [
                    "th",
    ...
    {
        return $this->closeIfCurrentMatches($ele, $current, ["li"]);
    }

    protected function handleDT($ele, $current)
    {
        return $this->closeIfCurrentMatches($ele, $current, ["dt", "dd"]);
    }

    protected function handleRT($ele, $current)
    {
        return $this->closeIfCurrentMatches($ele, $current, ["rt", "rp"]);
    }
