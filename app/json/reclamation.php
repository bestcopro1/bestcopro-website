...
if (isset($_POST["token"], $_POST["object"], $_POST["id"], $_POST["message"])) {
	...
	if ($token != "" && $objet != "" && $commentaire != "") {
		...
		if ($insert_stmt_history = $connection->prepare("INSERT INTO historique (date, action, id_collaborateur) VALUES (?, ?, ?)")) {
			...
			$insert_stmt_history->bind_param('sss', $date, $action, $lot[0]["id_proprietaire"]);
			// Execute the prepared query.
		...
		$date = date('Y-m-d H:i:s');
		if ($insert_stmt = $connection->prepare($request)) {
			$insert_stmt->bind_param('ssss', $date, $lot[0]["id_proprietaire"], $commentaire, $id_reclamation);
			// Execute the prepared query.
			if (! $insert_stmt->execute()) {
				echo $connection->error;
				exit();
		...
		if ($insert_stmt = $connection->prepare($request)) {
			$insert_stmt->bind_param('ssss', $date, $id_syndic, $reponseAuto, $id_reclamation);
			// Execute the prepared query.
			if (! $insert_stmt->execute()) {
				echo $connection->error;
				exit();
		...
		if ($insert_stmt = $connection->prepare($request)) {
			$insert_stmt->bind_param('sssss', $description, $date, $nomPage, $id_reclamation, $lot[0]["id_copropriete"]);
			// Execute the prepared query.
			if (! $insert_stmt->execute()) {
				echo $connection->error;
				exit();
		...
		if(isset($_FILES['file']['name'])){
			if ($_FILES['file']['name'] != "") {
				...
				$location = __DIR__."/../justificatifs/reclamations/".$id_reclamation.".".$fileType;
				move_uploaded_file($_FILES['file']['tmp_name'],$location);
			}
		}
		
	}
} elseif (isset($_GET["token"], $_GET["update"], $_GET["id"], $_GET["message"])) {
	...
	if ($token != "" && $update == "yes" && $id_reclamation != "" && $commentaire != "") {
...
<?php
		...
		foreach($reclamations as $reclamation):
			...
			$preuves = glob ("../justificatifs/reclamations/".$reclamation["id"].".*");
			if(count($preuves) == 0)
				$image = "https://bestcopro.ma/app/justificatifs/reclamations/no-image.jpg";
			else
				$image = "https://bestcopro.ma/app/".str_replace("../", "", $preuves[0]);
...
<?php
			...
			foreach($messages as $message) :
				if ($message["id_syndic"] != null) $source = "Syndic";
				else {
					$proprietaire = getProprietaire($message["id_proprietaire"], null, $connection);
					$source = $proprietaire[0]["civilite"]." ".$proprietaire[0]["prenom"]." ".$proprietaire[0]["nom"];
				}
?>
		{
			"id":"<?=$message["id"]?>",
			...
			"text" : "<?=str_replace(array( '<br>', '<br />', "\n", "\r" ), array( '', '', ' ', '' ), $message["commentaire"]);?>"
		}
<?php
				if (++$j < count($messages)) echo ",";
			endforeach;
?>
		]
    }
<?php
			if (++$i < count($reclamations)) echo ",";
		endforeach;
?>
]
<?php
