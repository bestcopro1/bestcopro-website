<?php
 if(!isset($_SESSION)) {
	session_start();
}

require_once '../vendor/dompdf/autoload.inc.php';

include_once(__DIR__.'/../config/db.php');
include_once(__DIR__.'/../controllers/functions.php');
$connection = $GLOBALS["connection"];

function getInfo($id_lot, $id_exercice, $connection){
    $request = "SELECT id_periodePaiement, SUM(partFonct)+SUM(partInv) FROM exercice, rel_lot_exercice WHERE id_lot = ? AND id_exercice = ? AND exercice.id = ?";
	if ($stmt = $connection->prepare($request)) {
		$stmt->bind_param('sss', $id_lot, $id_exercice, $id_exercice);
		$stmt->execute();
		$stmt->store_result();

		if ($stmt->num_rows > 0) {
			$stmt->bind_result(
				$id_periodePaiement,
				$sumPartFonctPartInv
			);
			while($stmt->fetch()){
				$result[] = 
					array(
						"id_periodePaiement" => $id_periodePaiement,
						"sumPartFonctPartInv" => $sumPartFonctPartInv
					);
			}
			return $result;
		} else {
			return [];
		}
	}
}

function getEtats($id_lot, $id_periodePaiement, $connection){
	$interval = "";
	if ($id_periodePaiement == "1")
		$interval = "1 MONTH";
	elseif ($id_periodePaiement == "2")
		$interval = "3 MONTH";
	elseif ($id_periodePaiement == "3")
		$interval = "6 MONTH";
	elseif ($id_periodePaiement == "4")
		$interval = "1 YEAR";
	$request = "SELECT id_lot, id_exercice, SUM(partFonct), SUM(partInv), SUM(cotisation) FROM rel_lot_exercice WHERE id_lot = ? AND DATE_SUB(dateFinPeriode, INTERVAL ".$interval.") <= CURDATE() GROUP BY id_exercice ORDER BY id_exercice DESC";
	if ($stmt = $connection->prepare($request)) {
		$stmt->bind_param('s', $id_lot);
		$stmt->execute();
		$stmt->store_result();

		if ($stmt->num_rows > 0) {
			$stmt->bind_result(
				$id_lot,
				$id_exercice,
				$sumPartFonct,
				$sumPartInv,
				$sumCotisation
			);
			while($stmt->fetch()){
				$result[] = 
					array(
						"id_lot" => $id_lot,
						"id_exercice" => $id_exercice,
						"sumPartFonct" => $sumPartFonct,
						"sumPartInv" => $sumPartInv,
						"sumCotisation" => $sumCotisation
					);
			}
			return $result;
		} else {
			return [];
		}
	}
}

// reference the Dompdf namespace
use Dompdf\Dompdf;

// instantiate and use the dompdf class
$dompdf = new Dompdf();

$id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_STRING);
$lot = getLot($id, null, null, $connection);
$copropriete = getCopropriete($lot[0]["id_copropriete"], $connection);
$typeLot = getTypelot($lot[0]["id_typeLot"], $connection);
$proprietaire = getProprietaire($lot[0]["id_proprietaire"], null, $connection);
$info = getInfo($lot[0]["id"], $_GET["id_exercice"], $connection);
$etats = getEtats($lot[0]["id"], $info[0]["id_periodePaiement"], $connection);
$paiements = getPaiement(null, null, $lot[0]["id"], $connection);

$htmlContent = '';
$htmlContent .= '<style> * { font-family: DejaVu Sans, sans-serif; } span, p {font-size: 10px;}</style>';
$htmlContent .= '<table style="width: 100%;">';
$htmlContent .= '<tr>';
$htmlContent .= '<td>';
$htmlContent .= '<img src="data:image/png;base64," alt="logo">';
$htmlContent .= '</td>';
$htmlContent .= '<td>';
$htmlContent .= '<strong>Situation de propriétaire</strong>';
$htmlContent .= '</td>';
$htmlContent .= '<td>';
$htmlContent .= '<span>'.date("d/m/Y").'</span>';
$htmlContent .= '</td>';
$htmlContent .= '</tr>';
$htmlContent .= '</table>';
$htmlContent .= '<p style="margin-top: 50px;">NB : Sauf erreur, omission, règlement en cours ou non identifié</p>';
$htmlContent .= '<div style="font-size: 12px;border: 1px solid #000;text-align: center;padding: 3px;margin-bottom: 10px;">';
$htmlContent .= '<strong>'.$copropriete[0]["nom"].'</strong>';
$htmlContent .= '</div>';

$htmlContent .= '<table style="width:100%;font-size: 12px;border-collapse: collapse;">';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">Nom & Prénom</td>';
$htmlContent .= '<td style="border: 1px solid #000;padding-left: 15px;font-weight: bold;" colspan="4">'.$proprietaire[0]["civilite"].' '.$proprietaire[0]["nom"].' '.$proprietaire[0]["prenom"].'</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="5"></td></tr>';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">Code '.$typeLot[0]["libelle"].'</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">'.$lot[0]["code"].'</td>';
$htmlContent .= '<td style="width: 1px;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">N° Immeuble</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">'.$lot[0]["numeroImm"].'</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="5"></td></tr>';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">N° '.$typeLot[0]["libelle"].'</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">'.$lot[0]["numero"].'</td>';
$htmlContent .= '<td style="width: 1px;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">N° Titre foncier</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">';'</td>';
if ($lot[0]["foncier"] != "") $htmlContent .= $lot[0]["foncier"]; else $htmlContent .= 'N/A';
$htmlContent .= '</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="5"></td></tr>';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">Téléphone</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">';
if ($proprietaire[0]["telephone"] != "") $htmlContent .= $proprietaire[0]["telephone"]; else $htmlContent .= 'N/A';
$htmlContent .= '</td>';
$htmlContent .= '<td style="width: 1px;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">E-mail</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">';
if ($proprietaire[0]["email"] != "") $htmlContent .= $proprietaire[0]["email"]; else $htmlContent .= 'N/A';
$htmlContent .= '</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="5"></td></tr>';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">Base des cotisations</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">'.number_format(floatval($info[0]["sumPartFonctPartInv"]),2).'</td>';
$htmlContent .= '<td style="width: 1px;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 120px;text-align: right;padding-right: 10px;background-color: #9cc2e5;">Tantième</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 180px;padding-left: 15px;font-weight: bold;">'.floatval($lot[0]["tantieme"]).'</td>';
$htmlContent .= '</tr>';
$htmlContent .= '</table>';

$htmlContent .= '<table style="width:100%;font-size: 12px;border-collapse: collapse;margin-top:20px;">';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="width: 33%;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;background-color: #92d050;">État des cotisations</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;background-color: #92d050;">État des impayés</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="3"></td></tr>';
$htmlContent .= '<tr>';
$totalCotisation = 0;
$totalImpaye = 0;
foreach($etats as $etat):
	if (intval($etat["id_exercice"]) < 0) :
		$exercice = getExercice($_GET["id_exercice"], null, $connection);
		$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;">Année '.date("Y",strtotime(date("Y-m-d", strtotime($exercice[0]["dateDebut"])) . " - ".abs($etat["id_exercice"])." year")).'</td>';
	elseif (intval($etat["id_exercice"]) === 0) :
		$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;">Impayé promoteur</td>';
	else :
		$exercice = getExercice($_GET["id_exercice"], null, $connection);
		$nameExercice = str_replace("Exercice ", "", getNameexercice($exercice[0]["dateDebut"]));
		$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;">Année '.$nameExercice.'</td>';
	endif;
	$totalCotisation += floatval($etat["sumCotisation"]);
	$totalImpaye += floatval($etat["sumPartFonct"]) + floatval($etat["sumPartInv"]) - floatval($etat["sumCotisation"]);
	$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;">'.$etat["sumCotisation"].'</td>';
	$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;">'.number_format(floatval($etat["sumPartFonct"]) + floatval($etat["sumPartInv"]) - floatval($etat["sumCotisation"]),2).'</td>';
	$htmlContent .= '</tr>';
	$htmlContent .= '<tr><td colspan="3"></td></tr>';
endforeach;
$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;"></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;"><strong>Total payé : '.number_format(floatval($totalCotisation),2).'</strong></td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 33%;text-align: center;"><strong>Total à payer : '.number_format(floatval($totalImpaye),2).'</strong></td>';
$htmlContent .= '</tr>';
$htmlContent .= '</table>';

$htmlContent .= '<table style="width:100%;font-size: 12px;border-collapse: collapse;margin-top:20px;">';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;text-align: center;background-color: #92d050;font-weight: bold;" colspan="4">Tableau des encaissements</td>';
$htmlContent .= '</tr>';
$htmlContent .= '<tr><td colspan="4"></td></tr>';
$htmlContent .= '<tr>';
$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;background-color: #c9c9c9;">Date de paiement</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;background-color: #c9c9c9;">Mode de paiement</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;background-color: #c9c9c9;">N° du reçu</td>';
$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;background-color: #c9c9c9;">Montant en MAD</td>';
$htmlContent .= '</tr>';
foreach($paiements as $paiement):
	$modepaiement = getModepaiement($paiement["id_modePaiement"], $connection);
	$htmlContent .= '<tr>';
	$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;">'.date("d/m/Y", strtotime($paiement["date"])).'</td>';
	$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;">'.$modepaiement[0]["libelle"].'</td>';
	$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;">'.date("y").sprintf("%'.05d", $paiement["id"]).'</td>';
	$htmlContent .= '<td style="border: 1px solid #000;width: 25%;text-align: center;">'.$paiement["montant"].'</td>';
	$htmlContent .= '</tr>';
endforeach;
$htmlContent .= '</table>';

//echo $htmlContent;
//exit();

$dompdf->loadHtml($htmlContent);

// (Optional) Setup the paper size and orientation
$dompdf->setPaper('A4', 'portrait');

// Render the HTML as PDF
$dompdf->render();

// Output the generated PDF to Browser
$dompdf->stream();