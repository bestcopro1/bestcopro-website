...
$request = 'SELECT reclamation.date, concat(proprietaire.civilite, " ", proprietaire.prenom, " ", proprietaire.nom) as nom_proprietaire, reclamation.objet, statutreclamation.libelle FROM reclamation, proprietaire, lot, statutreclamation WHERE reclamation.id_lot = lot.id AND lot.id_proprietaire = proprietaire.id AND reclamation.id_statutReclamation = statutreclamation.id AND lot.id_copropriete = '.$_GET["id_copropriete"].' AND reclamation.date BETWEEN "'.$exercice[0]["dateDebut"].'" AND "'.$exercice[0]["dateFin"].'"';
